    ...
    <div className="space-y-8 font-['Cairo']">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-2 flex items-center gap-1 font-medium">
              {data.regionMonthlyDonationsCount > 0
                ? <><span>عدد التبرعات هذا الشهر:</span><strong className="text-emerald-600 font-bold">{data.regionMonthlyDonationsCount}</strong></>
                : <><span>الحملات الإقليمية الجارية:</span><strong className="text-emerald-600 font-bold">{data.activeRegionCampaignsCount}</strong></>
              }
            </p>
          </div>
        </Link>
      ...
      <div className="bg-gradient-to-br from-emerald-500/10 via-card to-card border-2 border-emerald-500/30 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {data.pendingSermonsList.slice(0, 3).map((sermon) => (
            <div
              key={sermon.id}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* سجل العمليات والأنشطة الإدارية الحديثة - 8 Cols */}
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-base font-black text-foreground flex items-center gap-2">
              <Activity className="w-5 h-5 text-emerald-600" />
              <span>أحدث عمليات ونشاطات مساجد المنطقة</span>
            </h3>
            <Link href="/operations" className="text-xs font-bold text-emerald-600 hover:underline flex items-center gap-1">
              <span>عرض سجل العمليات الكامل</span>
              <ChevronLeft className="w-3.5 h-3.5" />
            </Link>
          </div>

          <div className="bg-card border border-border/80 rounded-3xl p-5 shadow-xs space-y-3">
            {(!data.recentOperations || data.recentOperations.length === 0) ? (
              ...
              data.recentOperations.slice(0, 5).map((op) => {
                const getBadge = (mod: string) => {
                  switch (mod) {
                    case 'donations':
                      return { label: 'تبرعات', color: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20', icon: DollarSign };
                    case 'maintenance':
                      return { label: 'صيانة', color: 'bg-amber-500/10 text-amber-600 border-amber-500/20', icon: Wrench };
                    case 'complaints':
                      return { label: 'شكاوى', color: 'bg-rose-500/10 text-rose-600 border-rose-500/20', icon: MessageSquareWarning };
                    case 'sermons':
                      return { label: 'خطب', color: 'bg-blue-500/10 text-blue-600 border-blue-500/20', icon: BookOpen };
                    default:
                      return { label: 'مسجد', color: 'bg-indigo-500/10 text-indigo-600 border-indigo-500/20', icon: Building2 };
                  }
                };

                const badge = getBadge(op.module);
                const BadgeIcon = badge.icon;

                let timeFormatted = op.created_at;
                try {
                ...
                return (
                  <div
                    key={op.id}
                    className="p-3.5 rounded-2xl bg-muted/25 hover:bg-muted/50 border border-border/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors"
                  >
                    <div className="flex items-start sm:items-center gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center border shrink-0 ${badge.color}`}>
                        <BadgeIcon className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg border ${badge.color}`}>
                            {badge.label}
                          </span>
                        ...
                        <p className="text-[11px] text-muted-foreground mt-1 flex items-center gap-2 flex-wrap">
                          {op.mosque_name && <span>{op.mosque_name}</span>}
                          {op.user_name && <span>• المنفذ: <strong className="text-foreground font-medium">{op.user_name}</strong></span>}
                          {op.amount !== undefined && (
                            <strong className="text-emerald-600 font-bold">
                              • {Number(op.amount).toLocaleString('ar-SA')} {op.currency || 'ل.س'}
                            </strong>
                          )}
                        </p>
                      </div>

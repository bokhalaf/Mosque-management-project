...
export function DownloadReportModal({
  ...
  // Fetch mosques catalog for dropdown
  useEffect(() => {
    if (!isOpen) return;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const url = `https://mms-backend-rose.vercel.app/api/admin/reports/${reportType}/download${queryString}`;
      ...
      if (response.ok && json?.data?.url) {
        ...
        onClose();
      } else if (response.ok && json?.report_url) {
        window.open(json.report_url, '_blank');
        onClose();
      } else if (response.ok && typeof json?.data === 'string' && json.data.startsWith('http')) {
        window.open(json.data, '_blank');
        onClose();
      } else {
        throw new Error(json?.message || 'فشل في إنشاء وتحميل ملف التقرير من السيرفر');
      }
    } catch (err: any) {
          ...
          >
            {downloading ? (
              <>
                ...
                <span>جاري معالجة وتوليد الـ PDF...</span>
              </>
            ) : (
              <>
                <FileDown className="w-4 h-4" />
                <span>تنزيل التقرير الآن (PDF)</span>
              </>
            )}
          </button>
        </div>

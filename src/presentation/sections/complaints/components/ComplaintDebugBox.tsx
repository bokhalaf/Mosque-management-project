...
export function ComplaintDebugBox({ debugData, copiedDebug, onCopy, onClose }: ComplaintDebugBoxProps) {
  ...
  const getMethodBadgeClass = (m: string) => {
    switch (m) {
      ...
      case 'PUT':
      case 'PATCH': return 'bg-amber-500/20 text-amber-300 border-amber-500/30';
  ...
  return (
    <div className={`p-6 bg-slate-900 border rounded-2xl shadow-2xl space-y-4 animate-in fade-in transition-all font-['Cairo'] ${
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <span className={`px-2.5 py-0.5 rounded text-[11px] font-mono font-black border ${getMethodBadgeClass(operationType)}`}>
              {operationType}
            </span>
            <h4 className="text-sm font-black text-white font-['Cairo']">
              مراقب السيرفر: {operationLabel} (HTTP {httpStatus})
            </h4>
          </div>
      ...
      <div className="space-y-3 text-xs font-mono">
        ...
        {/* Payload / Query Sent */}
        {requestPayloadSent && Object.keys(requestPayloadSent).length > 0 && (
          <div>
            <span className="text-slate-400 block mb-1"><strong>Request Query / Payload Sent (البيانات أو الفلاتر المرسلة):</strong></span>
            <pre className="p-3 bg-slate-950 text-amber-300 rounded-xl overflow-x-auto text-[11px] dir-ltr border border-amber-900/40 font-mono">
              {JSON.stringify(requestPayloadSent, null, 2)}
            </pre>
          </div>
        )}
        ...
        <div>
          <span className="text-slate-400 block mb-1"><strong>Server Returned Body Output (خرج الـ JSON القادم من السيرفر):</strong></span>

    ...
    <div className="flex flex-col min-h-screen bg-transparent font-['Cairo'] pb-12">
      <PageHeader 
        ...
        breadcrumbs={[
          { label: "إدارة المسجد" },
          { label: "خطب المسجد" },
          { label: "تفاصيل الخطبة", active: true }
        ]}
        actions={
          <div className="flex items-center gap-2">
            ...
            {sermon.status !== 'Scheduled' && (
              <button 
                onClick={() => {
                  if (onSelectForFriday) onSelectForFriday(sermon.id);
                  else alert(`تم تحديد خطبة "${sermon.title}" كخطبة الجمعة القادمة!`);
                }}
      ...
      <div className="px-4 md:px-8 py-4 grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-8 space-y-6">
          ...
          {/* SECTION 1: Sermon Title & Meta Header */}
          <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-6 border-t border-border">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-600 shrink-0 border border-emerald-500/20">
          ...
          {sermon.attachments && sermon.attachments.length > 0 && (
            <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-4">
              <h3 className="text-base font-black text-foreground flex items-center gap-2 border-b border-border pb-4">
                <Paperclip className="w-4.5 h-4.5 text-primary" /> المرفقات والمستندات ({sermon.attachments.length})
              </h3>
              <div className="flex flex-wrap gap-3">
                {sermon.attachments.map((att, idx) => {
          ...
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm sticky top-24 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <span className={`px-2.5 py-1 rounded-lg text-xs font-bold border inline-block ${badge.style}`}>
                  {badge.label}
                </span>
              </div>
              ...
              {sermon.notes && (
                <div className="pt-2 border-t border-border/60">
                  ...
                  <p className="text-xs text-foreground/90 font-medium leading-relaxed bg-muted/60 p-3 rounded-xl border border-border">
                    {sermon.notes}
                  </p>
                </div>
              )}

...
import { ApiDebugLog } from '../../../hooks/useDawahPrograms';

interface DawahDebugTerminalProps {
  show: boolean;
  onClose: () => void;
  logs: ApiDebugLog[];
  onClear: () => void;
...
export function DawahDebugTerminal({
  show,
  onClose,
  logs,
  onClear,
}: DawahDebugTerminalProps) {
  if (!show) return null;
      ...
      <div className="max-h-60 overflow-y-auto space-y-2.5 pr-1">
        {logs.length === 0 ? (
          <div className="text-slate-500 py-3 text-center">
            لا توجد طلبات مسجلة بعد. قم بتحديث البيانات أو إضافة برنامج/جلسة لمعاينة الطلبات الحية.
          </div>
        ) : (
          logs.map((log, i) => (
            <div
